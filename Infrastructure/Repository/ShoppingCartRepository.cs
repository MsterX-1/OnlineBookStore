       ...
       public async Task<IEnumerable<ShoppingCart>> GetCartItemsByCustomerIdAsync(int customerId)
        {
            ...
            return await db.QueryAsync<ShoppingCart>(sql, new { CustomerId = customerId });
		}
        public async Task<ShoppingCart?> GetCartItemAsync(int cartId)
        {
            using var db = _context.CreateConnection();
            var sql = "SELECT * FROM Shopping_Cart WHERE Cart_ID = @CartId";
            return await db.QueryFirstOrDefaultAsync<ShoppingCart>(sql, new { CartId = cartId });
		}
